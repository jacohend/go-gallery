...
func NewContractRepository(db *sql.DB) *ContractRepository {
	...
	getMetadataByAddressStmt, err := db.PrepareContext(ctx, `SELECT ID,VERSION,CREATED_AT,LAST_UPDATED,ADDRESS,SYMBOL,NAME,LATEST_BLOCK,OWNER_ADDRESS,CREATOR_ADDRESS,CONTRACT_URI,CONTRACT_METADATA FROM contracts WHERE ADDRESS = $1 AND DELETED = false;`)
	checkNoErr(err)
	...
	updateMetadataByAddressStmt, err := db.PrepareContext(ctx, `UPDATE contracts SET NAME = $2, SYMBOL = $3, OWNER_ADDRESS=$4, CREATOR_ADDRESS = $5, LATEST_BLOCK = $6, LAST_UPDATED = $7, CONTRACT_URI = $8, CONTRACT_METADATA = $9 WHERE ADDRESS = $1;`)
	checkNoErr(err)
	...
	return &ContractRepository{db: db, getByAddressStmt: getByAddressStmt, upsertByAddressStmt: upsertByAddressStmt, updateByAddressStmt: updateByAddressStmt, ownedByAddressStmt: ownedByAddressStmt, getMetadataByAddressStmt: getMetadataByAddressStmt, updateMetadataByAddressStmt: updateMetadataByAddressStmt}
...
func (c *ContractRepository) GetMetadataByAddress(pCtx context.Context, pAddress persist.EthereumAddress) (persist.Contract, error) {
	contract := persist.Contract{}
	err := c.getMetadataByAddressStmt.QueryRowContext(pCtx, pAddress).Scan(&contract.ID, &contract.Version, &contract.CreationTime, &contract.LastUpdated, &contract.Address, &contract.Symbol, &contract.Name, &contract.LatestBlock, &contract.OwnerAddress, &contract.CreatorAddress, &contract.ContractURI, &contract.ContractMetadata)
	if err != nil {
		return persist.Contract{}, err
	}
...
func (c *ContractRepository) UpdateMetadataByAddress(ctx context.Context, addr persist.EthereumAddress, up persist.Contract) error {
	if _, err := c.updateMetadataByAddressStmt.ExecContext(ctx, addr, up.Name, up.Symbol, up.OwnerAddress, up.CreatorAddress, up.LatestBlock, persist.LastUpdatedTime{}, up.ContractURI, up.ContractMetadata); err != nil {
		return err
	}
	return nil
}
